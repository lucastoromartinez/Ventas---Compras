...
compras_page = st.Page("pages/compras.py", title="Compras", icon="🧾")
ventas_page  = st.Page("pages/ventas.py",  title="Ventas",  icon="📊")

with col1:
    if st.button("🧾  IR A COMPRAS", use_container_width=True):
        st.switch_page(compras_page)

with col2:
    if st.button("📊  IR A VENTAS", use_container_width=True):
        st.switch_page(ventas_page)
